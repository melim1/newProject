...
<head>
  ...
  height: 100vh; /* La hauteur du conteneur parent est 100% de la hauteur de la fenêtre */
}
...
<body>
  <div class="container">
    ...
    <div class="card" id="card">
      <div class="card-body">
        <div class="swiper-container card-img-container">
...
<div id="rdv-form" class="modal">
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      const propertySwiper = new Swiper('.swiper-container', {
        loop: true,
        autoplay: {
          delay: 5000,
      ...
      // Gestion du carrousel de commentaires
      ...
      if (commentList && comments.length > 0) {
        const commentHeight = comments[0].offsetHeight + parseInt(window.getComputedStyle(comments[0]).marginBottom);
        const containerHeight = document.querySelector('.comment-list-container').offsetHeight;
        const visibleComments = Math.floor(containerHeight / commentHeight);
        let currentIndex = 0;
        const maxIndex = Math.max(0, comments.length - visibleComments);
        ...
        function slideComments() {
          commentList.style.transform = `translateY(-${currentIndex * commentHeight}px)`;
          updateButtons();
        }
        ...
        if (prevBtn && nextBtn) {
          ...
          commentList.addEventListener('touchmove', (e) => {
            ...
            if ((diff > 0 && currentIndex < maxIndex) || (diff < 0 && currentIndex > 0)) {
              e.preventDefault();
              currentY = y;
              commentList.style.transform = `translateY(calc(-${currentIndex * commentHeight}px - ${diff}px))`;
            }
          }, { passive: false });
          
          commentList.addEventListener('touchend', (e) => {
            if (!isScrolling) return;
            isScrolling = false;
            
            const diff = startY - currentY;
            const threshold = commentHeight / 3;
          ...
          if (confirm('Êtes-vous sûr de vouloir supprimer ce commentaire ?')) {
            ...
            .then(data => {
              if (data.success) {
                ...
                // Recalculer la position après suppression
